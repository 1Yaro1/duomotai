...
class DatasetForTransformer:
    def __init__(
        ...
        self.freq = freq
        self.__read_data__()
        self.tot_len = len(self.dataset) - self.history_length - self.prediction_length + 1
...
class TIMEMMDSegLoader(Dataset):
    def __init__(self, data, text, win_size, step, mode="train"):
        self.mode = mode
        self.step = step
        self.win_size = win_size
        ...
        self.text = text
        self.test_labels = data
        self.tot_len = len(self.data) - self.win_size + 1
    ...
    def __getitem__(self, index):
        ...
        if self.mode == "train":
            ...
            return train_data, train_label, index

        elif (self.mode == 'val'):
            val_data = np.float32(self.data[index:index + self.win_size])
            val_label = np.float32(self.test_labels[0:self.win_size])

            return val_data, val_label, index

        elif (self.mode == 'test'):
            test_data = np.float32(self.data[index:index + self.win_size])
            test_label = np.float32(self.test_labels[0:self.win_size])

            return test_data, test_label, index
...
def anomaly_detection_timeMMD_data_provider(data, text, batch_size, win_size=100, step=100, mode='train'):
    dataset = TIMEMMDSegLoader(data, text, win_size, 1, mode)
