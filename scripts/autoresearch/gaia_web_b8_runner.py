"""Web-only opt-in experiment. Default preflight; no test-set access or auto retries.
...
SERVICES = ["GAIA_webservice1", "GAIA_webservice2"]
CONFIG = ROOT / "config/autoresearch/gaia_web_b8_contrastive.json"
...
def configure():
    ...
    fixed = dict(services=SERVICES, seed=2021, batch_size=8, num_epochs=3,
                 ...
                 service_weighting="equal_over_exactly_two_web_instances")
    if any(candidate.get(k) != v for k, v in fixed.items()):
        raise ValueError("Candidate scope/protocol differs from approved web experiment")
    ...
    if [s["name"] for s in protocol["services"]] != SERVICES:
        raise ValueError("Missing/reordered web instances")
...
def summarize(output, protocol):
    ...
    for service in protocol["services"]:
        ...
        if row["service"] != service["name"] or any(not math.isfinite(row[k]) for k in frozen.METRICS):
            raise ValueError("Incomplete web result")
        selected.append(row)
    if [r["service"] for r in selected] != SERVICES:
        raise ValueError("Both web instances are mandatory; do not omit a failure")
    mean = {k: sum(row[k] for row in selected) / 2 for k in frozen.METRICS}
    frozen.write_csv(output / "best_metrics.csv", selected)
    summary = dict(scope="GAIA-web-dev (two instances, not full GAIA)", services=selected,
...
def run_worker(args, cache, protocol, baseline, candidate):
    ...
    logged_module.MindTSLogged = MindTSWeb
...
def main():
    ...
    if not args.run_validation and not args.replay_from:
        ...
        print(json.dumps({"web_preflight": "ok", "services": SERVICES, "batch_size": 8,
