...
def main():
    ...
    import numpy as np
    import torch
    from scripts.autoresearch.gaia_d1_b8_runner import configure
    ...
    params.update({k: candidate[k] for k in ("batch_size", "contrastive_enabled", "contrast_hidden", "contrast_dim", "contrast_dropout", "llm_memory_efficient")})
    ...
    terms = dual_contrast(views, torch.tensor(starts, device="cuda"), present,
                          candidate["contrast_temperature"], candidate["contrast_min_negatives"])
    loss = (out-x).square().mean() + baseline["lamda2"] * Bottleneck_loss(mask, baseline["r"], baseline["lamda"])
    loss = loss + candidate["intra_weight"]*(terms["intra_metric"]+terms["intra_log"])/2 + candidate["inter_weight"]*terms["inter"]
    loss.backward()
    ...
    optimizer.step()
    torch.cuda.synchronize()
    print(json.dumps({"d1_batch8_probe": "ok", "service": args.service, "loss": float(loss.detach()),
